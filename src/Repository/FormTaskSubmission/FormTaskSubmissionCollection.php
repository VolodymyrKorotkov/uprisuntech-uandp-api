...
final readonly class FormTaskSubmissionCollection implements \IteratorAggregate
{
    /**
     * @param array<FormTaskSubmission> $items
     */
    ...
    {
        return array_map(
            fn(FormTaskSubmission $submission) => $submission->getSubmissionId(),
            $this->items
        );
    }
     ...
     * @throws \Exception
     */
    public function getByForm(FormIo $form): FormTaskSubmission
    {
        foreach ($this->items as $processSubmission){
            if ($processSubmission->getForm() === $form){
