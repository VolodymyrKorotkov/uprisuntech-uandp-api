...
use App\Service\GovIdDecryptor\Dto\GovUaSenderInfo;
use function App\UserService\Service\GovIdDecryptor\euspe_develop;
use function App\UserService\Service\GovIdDecryptor\euspe_geterrdescr;
...
class EUSignCP
{
	...
	{
		$senderInfo = new GovUaSenderInfo();
		$data = '';
		$errorCode = EUSignCP::EU_ERROR_UNKNOWN;
		...
		$result = euspe_develop(
			...
			$senderInfo->subjCN,
			$senderInfo->subjOrg,
			$senderInfo->subjOrgUnit,
			$senderInfo->subjTitle,
			$senderInfo->subjState,
			$senderInfo->subjLocality,
			$senderInfo->subjFullName,
			$senderInfo->subjAddress,
			$senderInfo->subjPhone,
			$senderInfo->subjEMail,
			$senderInfo->subjDNS,
			$senderInfo->subjEDRPOUCode,
			$senderInfo->subjDRFOCode,
			$errorCode
		);
